...
public class MazeView extends JPanel{private MazeModel model;
...
protected void paintComponent(Graphics g) {
    ...
    // Calculer CELL_SIZE dynamiquement selon la taille de la fenêtre
    CELL_SIZE = Math.min(getWidth() / model.getMazeWidth(), getHeight() / model.getMazeHeight());
    int PLAYER_SIZE = CELL_SIZE - PLAYER_SIZE_RATIO;

    // Fond blanc pour effacer l’ancien dessin
    g2d.setColor(Color.WHITE);
    g2d.fillRect(0, 0, getWidth(), getHeight());

    // Dessiner les murs comme des briques
    ...
    // Centrer le joueur dans la cellule
